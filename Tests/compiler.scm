    ...
    (let ((run 
            (compose-patterns
                    ...
                    (pattern-rule
                        ...
                        (lambda (or-exps) 
                            (cond   
                                ((null? or-exps) (parse-2 #f))
                                ((equal? (length or-exps) 1) (parse-2 (car or-exps)))
                                (else
                                    `(or ,(map parse-2 or-exps))))))
                    ...
                    (pattern-rule
                        `(define ,(? 'varArg isArgsDefine?) ,(? 'exp1) . ,(? 'expRest))
                            (lambda (varArg exp1 expRest)
                                `(def (var ,(car varArg)) ,(parse-2 `(lambda ,(cdr varArg) ,exp1 . ,expRest)))))
                                
                    ;Assignmnet rule
                    (pattern-rule
                        `(set! ,(? 'varArg variable?) ,(? 'exp) )
                            (lambda (varArg exp)
                                `(set ,(parse-2 varArg) ,(parse-2 exp))))
                        ...
                        `(let ,(? 'let-def isLetDef?) . ,(? 'let-exp notNull?))
                         (lambda (let-def let-exp)
                            (let ((var-values (arrangeLetVarValue let-def '() '())))
                                (parse-2 `( (lambda ,(car var-values) ,@let-exp) 
                                                 ,@(cdr var-values))))))
